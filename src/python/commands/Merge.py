...
class Merge:
	...
	def merge(self, origin, dest):
		...
		while filequeue:
			...
			origin_filename = os.path.join(origin, file_name)
			dest_filename = os.path.join(dest, file_name)

			try:
				if os.path.isdir(origin_filename) and not base_name in self.EXCLUDE:
					filequeue.extend(os.path.join(file_name, ch) for ch in os.listdir(origin_filename))
				elif not os.path.isfile(origin_filename):
